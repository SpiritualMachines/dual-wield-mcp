...
- `clipboard_set(text)` / `clipboard_get()` — set or read the Wayland clipboard via `wl-copy`/`wl-paste`. Prefer `clipboard_set` + `key_press("ctrl+v")` over `type_text` for long or special-character strings (URLs, search queries) — pasting is a single atomic operation instead of simulated per-character typing.
